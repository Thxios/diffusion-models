...
class GuidedPredictor(BasePredictor):
    ...
    def get_pred_fn(self, cond=None, guidance_scale=1.0, **pred_kwargs):
        def pred_fn(z, t):
            if cond is None or guidance_scale == 0.0:
                pred = self.pred_conditional(z, t, **pred_kwargs)
            elif guidance_scale == 1.0:
                pred = self.pred_conditional(z, t, cond=cond, **pred_kwargs)
            else:
